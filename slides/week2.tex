...
An effect-oriented \textbf{functional}, \textbf{imperative}, and \textbf{logic}
...
\begin{itemize}
    ...
    \item traits and automatic derivation
    \item higher-kinded types
    \item associated types and effects
\end{itemize}
\end{column}
\begin{column}{.45\textwidth}
\begin{itemize}
    \item algebraic effects and handlers
    \item default effect handlers
    \item purity reflection
    \item region-based local mutation
    \item structured concurrency
    \item first-class Datalog constraints
    \item and more ...
\end{itemize}
\end{column}
\end{columns}
...
We can compute the ancestors of every person:
...
\begin{itemize}
    ...
    \item That is, we have extension by \emph{addition}, not by \emph{modification}.
\end{itemize}
\end{frame}
...
\begin{center}
\large
\textbf{Idea:} Datalog programs as \emph{first-class} values.
\end{center}
\end{frame}
...
We can use the \Code{getAncestors} function as follows:
...
\begin{itemize}
    \item The solution to a Datalog program value is its minimal model.
    \item The minimal model is itself a Datalog program value.
\end{itemize}
\end{frame}

\begin{frame}[fragile]{Datalog Literals (1/2)}
...
\begin{frame}[fragile]{Datalog Literals (2/2)}
...
A Datalog program is \emph{inert} until its minimal model is computed with
\Code{query}. 

\begin{itemize}
    \item That is, in the above Datalog literal the fact \lstinline{C(1)} is
...
Any type that implements \Code{Eq[t]} and \Code{Order[t]} can be used as a
term.

\pause

\textbf{Q:} What types are then excluded?
\end{frame}

\begin{frame}[fragile]{Lexical Scope (1/2)}
Datalog literals integrate with lexical scope.

We can capture variables from lexical scope:
...
\begin{lstlisting}[language=flix, xleftmargin=0.8cm]
def mkParentsOfAll(l: List[(String, String)]): #{ ... } = 
    match l {
        case Nil          => #{}
        case (c, p) :: xs => 
            #{ ParentOf(c, p). } <+> mkParentsOfAll(xs)
    }
\end{lstlisting}
...
\begin{itemize}
    \item Functional languages use data structures: lists, sets, and maps.
...
We introduce the \Code{inject} construct as a mechanism to associate a collection
...
\begin{itemize}
    \item E.g. \texttt{List[t]}, \texttt{Set[t]}, \texttt{Vector[t]}, \texttt{Chain[t]}, and many more...
\end{itemize}
...
which evaluates to the union of the constraints in both $e_1$ and $e_2$.
...
We can use \Code{query} to compose them and compute their minimal model:
...
\begin{itemize}
    \item We can use \Code{inject} to translate any data type that implements
...
\begin{frame}[fragile]{Overview: Fundamental Operations}
...
\begin{itemize}
    \small
    \item The \Code{inject e1, ..., en into A1/X1, ..., An/Xn} expression returns a
    Datalog program where each element of the collection $e_i$ becomes a fact with
...
\begin{itemize}
    \small
    \item The \Code{query e1, ..., en select (x1, ..., xm) from A1, ..., Ak}
    ...
    $A_1, \cdots, A_k$. The result is a \Code{Vector} of tuples.
\end{itemize}
\end{frame}
...
% - Types: x is captured by lexical scope in y and z -- and so is y in z.
%          (A let-bound name is not in scope in its own definition, so the
%           y in y and the z in z are genuine Datalog variables.)
...
\begin{itemize}
    ...
    \item (\textbf{Compilation}) Types help the compiler generate efficient code.
    \item (\textbf{IDE Support}) Autocompletion, automatic refactoring, etc.
\end{itemize}

\medskip

\small
Flix also has a powerful \emph{effect system}, which we sadly do not have time to discuss.
\end{frame}
...
\begin{lstlisting}[language=flix, xleftmargin=0.8cm]
>> Unable to unify the types: 'Edge/2' and 'Edge/3'.
...
$$
where $\tau$ ranges over the base types (e.g. \Code{Bool}, \Code{Int32}, \Code{String}).

\medskip

\pause

We consider rows equivalent up to permutation of distinct labels.
\end{frame}
...
We should have used \textbf{open rows}:
...
Open rows can be extended with additional predicates, allowing us to combine the results of \Code{f} and \Code{g}.
\end{frame}
...
\textbf{Observation:} The \Code{Vertex} and \Code{Path} predicates are internal
...
\small
\begin{itemize}
    \item A rho abstraction is of the form \Code{#(A, ...) -> e} where $e$ is a Datalog expression.
    \pause \item A rho abstraction hides, by renaming, all predicates \emph{not}
...
\textbf{Example:} The rho abstraction:
%
\begin{lstlisting}[language=flix, xleftmargin=0.5cm]
#(A, B) -> #{A(123). C("a").}
\end{lstlisting}
%
evaluates to \Code|#{A(123). C17("a").}| where \Code{C17} is a fresh predicate
symbol.
\end{frame}

\begin{frame}[fragile]{Rho Abstraction: The Wrong Way}
We may erroneously think that we can simply statically rename predicate symbols:
...
Every evaluation of a rho abstraction introduces fresh names.
...
\textbf{Upshot:} The abstracted predicate symbols have become truly local.
\end{frame}
...
In that case, we compute their \emph{join}, which, according to the reversed lattice
...
\begin{itemize}
    \item It now also supports \emph{constraints on lattices}.
\end{itemize}
...
A \emph{bounded} lattice has the following components:
...
\small
\textbf{Note:} For the fixpoint to terminate, the lattice must have no infinite
ascending chains.
...
\begin{frame}[fragile]{Lattice Stratification}
...
\begin{frame}[fragile]{Motivation (1/2)}
We have seen how to compute the transitive closure of a graph:
...
def main(): Unit \ IO =
    ...
    let result = query pr select (x, y) from Path(x, y);
    println(result)
\end{lstlisting}
...
The minimal model contains \Code{Path(1, 4)}; hence, a path \emph{exists} from
$1$ to $4$.

\pause

{\large \textbf{But what if we wanted to compute the \emph{path} itself?}}
\end{frame}

\begin{frame}{Motivation (2/2)}
The minimal model tells us \emph{that} a fact holds, but not \emph{why} it holds.

The same problem appears in many domains:

\begin{itemize}
    \pause \item \textbf{Route planning:} A route exists from Paris to Rome. \\
    $\Rightarrow$ \textcolor{red}{\bfseries Which roads should we follow?}
    \pause \item \textbf{Taint analysis:} User input flows into an SQL query. \\
    $\Rightarrow$ \textcolor{red}{\bfseries Along what dataflow path?}
    \pause \item \textbf{Cyclic inheritance:} A Java program has a cycle in its
    class hierarchy. \\
    $\Rightarrow$ \textcolor{red}{\bfseries Which classes form the cycle?}
\end{itemize}
...
\begin{center}
\Large
\textbf{Solution:} We need \emph{provenance} --- \emph{how} a fact was derived.
\end{center}
\end{frame}

\begin{frame}{Provenance Trees}
Given a Datalog program $P$ and a fact $A$ in its minimal model, a
\textbf{provenance tree} for $A$ records its complete derivation:

\begin{itemize}
    \item The \textbf{leaves} are EDB facts, i.e. input facts.
    \item The \textbf{internal nodes} are IDB facts, i.e. derived facts,
    each annotated with the rule by which it was derived.
\end{itemize}

\pause

The minimal model of $P$ is \emph{unique}, but a fact $A$ may have
\emph{multiple} provenance trees --- e.g. there may be several distinct paths
between two vertices in a graph.
\end{frame}

\begin{frame}[fragile]{\only<1>{Provenance Trees: Example}\only<2->{Provenance Paths}}
\vspace{0.5cm}

\begin{overlayarea}{\textwidth}{2.7cm}
\begin{onlyenv}<1>
...
A provenance tree for the fact \Code{Path(1, 4)}:
\end{onlyenv}
\begin{onlyenv}<2->
Given a fact $A$ and a set of query predicate symbols $\Pi$, a
\textbf{provenance path} for $A$ is the sequence of facts with a predicate symbol
in $\Pi$ obtained by a depth-first, left-to-right traversal of a provenance tree
for $A$.

\medskip

For example, for the fact \Code{Path(1, 4)} and $\Pi = \{ \textsf{Edge} \}$:
\end{onlyenv}
\end{overlayarea}
...
EDB/.style={fill=gray!20}
[{$\textsf{Path}(1, 4)$}, label={left:{\scriptsize\texttt{\textbf{R2}}}}
    [{$\textsf{Path}(1, 3)$}, label={left:{\scriptsize\texttt{\textbf{R2}}}}
        [{$\textsf{Path}(1, 2)$}, label={left:{\scriptsize\texttt{\textbf{R1}}}}
            [{$\textsf{Edge}(1, 2)$}, EDB, name=e12]
        ]
        [{$\textsf{Edge}(2, 3)$}, EDB, name=e23]
    ]
    [{$\textsf{Edge}(3, 4)$}, EDB, name=e34]
]
\node<1>[overlay, anchor=north west, align=left] at ([xshift=2.5em, yshift=-1.6em]e23.north -| e34.east) {
    \fcolorbox{black}{gray!20}{\makebox[2.2em]{EDB}} facts in gray \\
    \fcolorbox{black}{white}{\makebox[2.2em]{IDB}} facts in white
};
\draw<2->[overlay, dashed, bend right=45, -latex, red, thick] (e12) to (e23);
\draw<2->[overlay, dashed, bend right=45, -latex, red, thick] (e23) to (e34);
\node<2->[overlay, anchor=north west, align=left, red, font=\itshape]
    at ([xshift=2.5em, yshift=-1.6em]e23.north -| e34.east)
    {the provenance path};
\end{forest}
\end{center}
\end{frame}

\begin{frame}[fragile]{The \texttt{pquery} Construct}
We extend Flix with a \textbf{provenance query} construct:

\begin{lstlisting}[language=flix, xleftmargin=0.8cm]
pquery e1, ..., en select F with {A1, ..., Am}
\end{lstlisting}

which computes the minimal model of the Datalog program values $e_1, \cdots,
e_n$, finds a provenance tree for the fact $F$, and returns its provenance path
w.r.t. the predicate symbols $A_1, \cdots, A_m$ as a \Code{Vector} of facts.
\end{frame}

\begin{frame}[fragile]{Example: Computing a Path in a Graph}
We can now compute the path from the motivating example:
...
def main(): Unit \ IO =
    ...
    let p = pquery pr select Path(1, 4) with {Edge};
    foreach(fact <- p) {
        ematch fact { case Edge(x, y) => println("${x} -> ${y}") }
    }
\end{lstlisting}

which prints \Code{1 -> 2}, \Code{2 -> 3}, and \Code{3 -> 4}.
\end{frame}

\begin{frame}[fragile]{Example: Cyclic Inheritance (1/2)}
We can use \Code{query} and \Code{pquery} to detect \emph{and explain} cyclic
inheritance in Java:

\begin{lstlisting}[language=flix, xleftmargin=0.8cm]
let db = #{ /* ... Extends and Implements facts ... */ };
let pr = #{
...
We use \Code{query} to compute every type that is part of a cycle.
\end{frame}

\begin{frame}[fragile]{Example: Cyclic Inheritance (2/2)}
For each such type, we use \Code{pquery} to explain its cycle:
...
foreach (typ <- cycles) {
    ...
    foreach (edge <- p) {
        ematch edge {
            case Extends(x, y)    => println("  ${x} extends ${y}")
            case Implements(x, y) => println("  ${x} implements ${y}")
        }
    }
}
\end{lstlisting}

The provenance path mixes \Code{Extends} and \Code{Implements} facts; hence, the
pattern match must handle both cases.
\end{frame}

\begin{frame}[fragile]{Example: Road Tolls}
Consider a road network where each road has a toll:

\begin{lstlisting}[language=flix, xleftmargin=0.8cm]
let db = #{ /* ... Road and Toll facts ... */ };
let pr = #{
    Path(x, y) :- Road(x, y), Toll(x, y, cost).
    Path(x, z) :- Path(x, y), Road(y, z), Toll(y, z, cost).
};
\end{lstlisting}

We can compute the total toll along one route from Toronto to Chicago:

\begin{lstlisting}[language=flix, xleftmargin=0.8cm]
let p = pquery db, pr select Path("Toronto", "Chicago") with {Toll};
Vector.sumWith(ematch Toll(x, y, cost) -> cost, p)
\end{lstlisting}

\pause

\textbf{Upshot:} We are not always interested in the path itself; sometimes we
want to compute over the facts \emph{used} to derive a fact --- for \emph{some}
derivation, since a fact may have many.
\end{frame}

\begin{frame}{Provenance}
\textbf{Provenance} explains \emph{how} a fact in the minimal model was derived:

\begin{itemize}
    \item A \textbf{provenance tree} records the complete derivation of a fact:
    its leaves are EDB facts and its internal nodes are IDB facts.
    \item A \textbf{provenance path} is the sequence of facts, restricted to a
    chosen set of predicate symbols, obtained by a depth-first, left-to-right
    traversal of a provenance tree.
    \item The \Code{pquery} construct computes provenance paths for first-class
    Datalog.
\end{itemize}

We can compute not only \textbf{\emph{what}} holds, but \textbf{\emph{why}} it
holds.
\end{frame}
...
\begin{itemize}
    ...
    lattices}.
    \item How to compute provenance information with \lstinline{pquery}.
\end{itemize}
\end{frame}
