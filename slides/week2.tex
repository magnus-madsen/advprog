...
\textbf{Upshot:} Using \Code{query} and \Code{inject}, the model of one program
can feed the next --- we can build pipelines of Datalog programs.
\end{frame}
...
\begin{itemize}
    ...
    \item We can store them inside data structures (e.g., lists and maps).
\end{itemize}
...
\begin{itemize}
    \item In particular, they do \emph{not} implement \Code{Eq[t]},
    \Code{Order[t]}, or \Code{ToString[t]}.
    \item Hence, we can only operate on them with \Code|<+>| and \Code{query}.
\end{itemize}
\end{frame}
...
\begin{lstlisting}[language=flix, xleftmargin=0.8cm]
def mkParentOfAll(l: List[(String, String)]): #{ ... } = 
    match l {
        case Nil          => #{}
        case (c, p) :: xs => 
            #{ ParentOf(c, p). } <+> mkParentOfAll(xs)
    }
\end{lstlisting}
