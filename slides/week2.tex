...
\begin{frame}{A Polymorphic Type Systems}
...
\]
%
Hence the composition \Code{p1 <+> p2} has the type:
%
\[
...
\begin{itemize}
    \item Asserts that \Code{Dist} is a (map) lattice, and \emph{not} a relation (the semicolon).
    \item Asserts that the distance to the origin is at most zero. 
    \item The \Code{Down} data type, which wraps zero, \emph{reverses} the order
    on \Code{Int32}.
\end{itemize}
\end{frame}
...
%
\begin{itemize}
    \item Asserts that \Code{Dist} is a (map) lattice, and \emph{not} a relation (as before).
    \item Asserts that the distance to \Code{y} is \emph{at
