...
\begin{tikzpicture}
    ...
    \node (3) [below = 0.3cm,at=(2b.south), xshift=-3cm]
      {\texttt{ys\_first[0]+ys\_last[0] \ldots~ys\_first[n/2-1]+ys\_last[n/2-1]}};
    ...
    \node (6) [below = 0.3cm,at=(5b.south), xshift=-3cm]
      {\texttt{ys\_first[0]+ys\_last[0]}};
...
\begin{frame}
  ...
  \item Compelling because it fits hardware well, but is also semantically easy
    to understand for humans because of its determinism.
  ...
  \item Futhark is not the only language for data parallel programming, but it
    ...
    \end{itemize}
  \item You can also do data parallel programming in normal languages, but it
    may not run so fast.
  \end{itemize}

  \hfill\includegraphics[width=4cm]{img/hedgehog.jpg}
