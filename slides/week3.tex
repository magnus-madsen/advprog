...
\begin{frame}{Prolog}
\Large
Prolog: \emph{Pro}grammation en \emph{Log}ique
...
\begin{itemize}
    ...
    \item Datalog has limited expressive power.
\end{itemize}
...
% Copied from week1.tex
\centering
\medskip
\medskip
\begin{tikzpicture}[font=\sffamily, scale=1.0]
\foreach \X/\col/\LY [count=\N, evaluate=\N as \Y using 4-\N] in {Prolog/blue/-3.0, Datalog$^\neg$/green/-2.0, Relational\\Algebra/orange/-0.5}
{\draw[fill=\col!30] (-\Y,-\Y/2) circle ({1.5*\Y} and \Y);
\node[align=center] at (1-2*\Y,\LY) {\X}; }
...
Prolog computes a \textbf{\emph{single}} solution at a time, answering \textbf{yes} or \textbf{no}.
...
\begin{itemize}
    \item Allows us to construct compound values (lists, trees, etc.).
...
\begin{lstlisting}[language=prolog, xleftmargin=0.5cm]
networth(person(steve, carell),   80). % in millions USD
...
\begin{lstlisting}[language=prolog, xleftmargin=0.5cm]
appnd(nil, Ys, Ys).
appnd(cons(X, Xs), Ys, cons(X, Rs)) :- appnd(Xs, Ys, Rs).
\end{lstlisting}
...
because we wanted to force Prolog to evaluate \Code{N + 1} and unify the result with \Code{R}. 
...
The core Prolog grammar is almost identical to the Datalog grammar:
{
\large
\begin{align*}
...
\\
p \in \textsl{Predicates}   & = \text{a finite set of predicate symbols.} \\
x \in \textsl{Variables}    & = \text{a finite set of variable symbols.} \\
\Highlight{c \in \textsl{Constructors}} & = \Highlight{\text{a finite set of constructors.}} \\
k \in \textsl{Constants}    & = \text{a finite set of constants.}
\end{align*}
}
\end{frame}
...
\textbf{Side note:} A cool idea is when $=$ is replaced by $\equiv$ leading to
...
\begin{lstlisting}[language=flix, xleftmargin=0.5cm]
def unify(t1: Term, t2: Term): Subst \ Abort = match (t1, t2) {
    ...
    case (Term.Pair(t11, t12), Term.Pair(t21, t22)) => 
        ...
        compose(s1, s2)
    case _ => Abort.abort("Unification failed")
}
\end{lstlisting}
...
\textbf{Oops!} This is incorrect. What's the problem?

\end{frame}

\begin{frame}[fragile]{A Unification Algorithm (in Flix) (6/6)}
We modify the cases for variables as follows: 

\begin{lstlisting}[language=flix, xleftmargin=0.5cm]
case (Term.Var(x), Term.Var(y)) if x == y => Map.empty()
case (Term.Var(x), t) => 
    if (Set.memberOf(x, freeVars(t)))
        Abort.abort("Occurs Check")
    else 
        Map.singleton(x, t)
...
\textbf{Note:} The occurs check is expensive, so most Prolog implementations omit it by default.
\end{frame}  
...
\begin{enumerate}
    ...
    substitution $\{ \text{\_Head} \mapsto 1, \text{Tail} \mapsto [2 | []], R \mapsto X \}$. \pause
    \item We then apply the substitution to the rule body to obtain the new
    goals: $\text{len}([2 | []], N)$ and $X \text{ is } N + 1$, and recurse.
    Each use of a rule works on a \emph{fresh} copy of its variables
    (\emph{renaming apart}).
\end{enumerate}
\end{frame}
...
\begin{itemize}
    ...
    \pause \item If a match is found, Prolog applies the found substitution to the rule
    body, which now becomes the sub-goals of the query. 
    \pause \item If no match is found, or a later sub-goal fails, Prolog
    \emph{backtracks}: it undoes the substitution and tries the next matching
    fact or rule.
...
Print debugging is back, baby! We can write:
...
\textbf{Question:} What does \Code{?- path(1, 3).} print?
...
We can explore Prolog's evaluation order with the program:

\begin{lstlisting}[language=prolog, xleftmargin=0.5cm]
edge(1, 2).
edge(2, 3).
path(X, Z) :-
    write('A'), edge(X, Y), write('B'), path(Y, Z), write('C').
\end{lstlisting}
...
\begin{itemize}
    ...
    \item Tabling (a mechanism to memoize goals)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{XSB Prolog} is an open-source Prolog implementation with tabling
...
\begin{itemize}
    ...
    \item Ciao is a large research project with many cool ideas. 
...
\Large
Run the parent/grandparent program from the previous slide.
...
We further assume we have four constants: \Code{farmer}, \Code{wolf},
...
\begin{itemize}
    ...
    \item The list: \Code{[e, e, e, w]} means that the farmer, wolf, and goat are
    on the east bank whereas the cabbage is on the west bank --- which is okay.
    \item The list: \Code{[e, w, w, e]} means that the farmer and cabbage are on
    the east bank whereas the wolf and the goat are on the west bank --- which is
    NOT OKAY!
\end{itemize}
\end{frame}
    
\begin{frame}[fragile]{Wolf, Goat, and Cabbage in Prolog (3/8)}
We now define a ternary relation: \Code{move(state1, moved, state2)}:
...
\textbf{Recall:} We want to ensure that (a) the wolf does not eat the goat, and
...
How? We fix the length of the list of moves:
...
%
\begin{itemize}
    \item Datalog computes all solutions. Prolog searches for one. 
\end{itemize}
