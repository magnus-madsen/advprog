...
\begin{itemize}
    ...
    \pause \item Use algebraic simplifications (divide by $2$ on both sides, and so on).
...
A constant $k$ is a primitive value, e.g. a number or string.
\end{frame}
...
A \textbf{rule} is said to be \textbf{ground} if all of its atoms are ground.
...
\begin{itemize}
    ...
    \item Every Datalog program can be evaluated in polynomial time.
\end{itemize}
...
\begin{itemize}
    ...
    \item Souffl\'e is an open source and highly scalable Datalog engine {\small
...
\begin{itemize}
    \item Flix is a full-blown functional, logic, and imperative programming
    language.
    ...
    Copenhagen, and the University of T\"ubingen (Germany).
\end{itemize}  
\end{frame}
...
\begin{itemize}
    ...
    \item A model is an interpretation that satisfies all constraints in the program.
...
\begin{frame}{Model-theoretic Semantics (2/2)}
...
\Large
$$
\mathcal{U} = \{ \textsf{"Bart"}, \textsf{"Lisa"}, \textsf{"Homer"}, \textsf{"Grampa"} \}
$$
\end{frame}

\begin{frame}{Herbrand Base}
The Herbrand Base $\mathcal{B}$ of a Datalog program $P$ is the set of all
ground atoms with predicate symbols drawn from $P$ and terms drawn from the
...
What we \emph{lack} is a method to \textbf{compute} the minimal model of a program.
\begin{itemize}
...
atoms of each ground rule instance whose body is satisfied by $I$. For example, if we have the
interpretation:
$$
I = \left\{
\begin{array}{l}
...
\text{Iteration i} &= T_P^i(\emptyset) = T_P(T_P^{i-1}(\emptyset))
\end{align*}
%
...
\begin{frame}{Datalog Grammar Extended with Negation}
...
$$
%
Both are minimal, but neither is contained in the other --- there is no
unique minimal model! Yikes!
\end{frame}
...
\draw[->, line width=1.25pt] (8,1) -- (9,1);
...
\begin{itemize}
    \item Critically, we must partition the predicates carefully such that
    there is no circular dependency with negation.
\end{itemize}
\end{frame}
...
%
$$
P_1 = \{ \textsf{Edge}, \textsf{Path}, \textsf{Vertex} \}
\qquad \text{and} \qquad
P_2 = \{ \textsf{Unconnected} \}
$$
\end{frame}
